...
export default function PesertaNavbar() {
  ...
  const confirmLogout = async () => {
    ...
    try {
      // Panggil Server Action untuk logout
      await logout();
      // Server Action akan melakukan redirect, jadi tidak perlu
      // kode tambahan di sini (seperti window.location atau router.push).
    } catch (error) {
      console.error("Gagal memanggil logout action:", error);
                      ...
                      {pelatihanDropdown.map((item, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isActive && <div className="w-2 h-2 rounded-full bg-gradient-to-r from-gold to-yellow-500 animate-pulse shadow-sm"></div>}
                          </button>
                        );
                      })}
                    </div>
          ...
          <div className="hidden lg:block relative flex-shrink-0">
            ...
            >
              <div className="w-8 h-8 bg-gradient-to-br from-gold to-yellow-600 text-navy rounded-full flex items-center justify-center text-xs font-semibold transition-transform duration-200 group-hover:scale-110 shadow-lg">
                {/* Access email directly from user */}
                {user?.email?.charAt(0).toUpperCase() || "U"}
              </div>
              <span className="max-w-32 truncate text-white-text">
                {/* Access email directly from user */}
                {user?.email || "User"}
                {/* Optionally, display the user's full name from the profile if available: */}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="font-semibold text-navy text-sm gradient-text">
                          {/* Corrected path: Access name from profile */}
                          {user?.profile?.nama_lengkap || user?.email?.split("@")[0] || "Peserta"}
                        </div>
                        <div className="text-xs text-gray-500 truncate">
                          {/* Corrected path: Access email directly */}
                          {user?.email}
                        </div>
                        <div className="mt-1">
                          {/* RoleIndicator already uses useAuth internally or receives role as prop */}
                          <RoleIndicator />
                        </div>
                      </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-gold/30 bg-navy">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <div className="border-t border-gold/30 pt-4 mt-4">
                <div className="px-3 py-2 text-sm text-silver font-medium">Akun</div>
                <button
                  onClick={() => {
                    setMobileMenuOpen(false);
                    handleNavigation("/peserta/profil");
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 mx-4 max-w-md w-full transform transition-all duration-300 animate-slideInFromTop">
            ...
            {/* Content */}
            <div className="mb-6">
              <p className="text-gray-700 leading-relaxed">Apakah Anda yakin ingin keluar dari akun?</p>
              <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <p className="text-sm text-amber-800">⚠️ Anda akan diarahkan kembali ke halaman utama dan perlu login ulang untuk mengakses dashboard peserta.</p>
              </div>
            </div>
